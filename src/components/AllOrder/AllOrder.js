...
const AllOrder = ({deleteOrderFromAllOrder , order}) => {
    const deleteOrder = ()=>{
        axios.delete(`${process.env.REACT_APP_BACKEND_URL}/all-orders/${order._id}`)
        .then(res => {
            if (res?.data?.acknowledged) {
                deleteOrderFromAllOrder(order._id)
            ...
            <div className="p-6 mx-auto bg-white border rounded-lg shadow-xl lg:w-1/2">
                <div className="flex flex-col items-start py-2 rounded-lg lg:flex-row">
                    ...
                    <div className="flex flex-col w-full text-blueGray-500 lg:ml-4">
                        ...
                        <h2 className="mt-4 mb-8 text-xs font-semibold tracking-widest text-black uppercase lg:mt-0 title-font font-bold">{order.useremail}</h2>
